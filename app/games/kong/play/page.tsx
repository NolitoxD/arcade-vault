...
const KongGame = dynamic(() => import('@/components/games/KongGame'), {
  ssr: false,
});

const keyMap = getKeyMap('kong');
const MAX_LIVES = 3;
...
// pattern as the hearts HUD in space-invaders/karate-champ play pages.
...
export default function KongPlay() {
  const { username, saveScore } = useUser();
  const { setTrackOverride } = useMusic();
  const { skinKey, options, change } = useGameSkin('kong');
  ...
  useEffect(() => {
    setTrackOverride('/kong-theme.mp3');
    ...
    <div className="av-player fade-in">
      <div className="hidden md:block">
        <div className="player-hud">
          ...
          <div className="hud-actions">
            ...
            <Link href="/games/kong" className="btn ghost">
              SALIR
            </Link>
          </div>
        </div>
      ...
      {helpOpen && (
        ...
        >
          ...
          >
            <InstructionsContent game={getGame('kong')!} title="KONG" />
            <div className="actions">
