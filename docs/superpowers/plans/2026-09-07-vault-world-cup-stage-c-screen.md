...
14. Reducir la ventana por debajo de 900 × 600 **en pleno partido**. Lo que tiene que pasar, en este orden y **todo dentro del canvas, que sigue dibujándose** (no se congela en el frame anterior):
    ...
    - debajo de esa banda, un panel fijo: **`AGRANDA LA VENTANA` / `MÍNIMO 900 × 600`**;
    - el «Estado» de la página coincide, también `EMPATE`.

    Probar además el **caso feo**: entrar en la ruta con la ventana ya por debajo del umbral. El canvas **no puede quedarse en blanco** — el panel tiene que estar ahí desde el primer frame. El *redirect* y el «deshabilitado en el catálogo» son del paso 10. ¿Es 900 × 600 el umbral que quieres? (S-SC2.)
...
| **S-SC2** | **Viewport mínimo 900 × 600** (`VIEW_W + 100`, `VIEW_H + 100`). Por debajo, el partido se para con `abandon()`. **El repo no tiene ninguna guarda de viewport**: estos son los dos primeros umbrales del proyecto. | El breakpoint `lg` de Tailwind (1024). Descartado: dejaría fuera portátiles de 13" en ventana partida sin necesidad. |
